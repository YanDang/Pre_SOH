...
def pre_data(df,index_l,index_r,rf_end_V,rf,end_index):
    ...
    x_pre = [list(pre_end_V) + x_pre_V[0] + [SOC_deplete]]
    pre_mileage = rf.predict(x_pre)
    pre_total_mileage = pre_mileage + df['累计里程'][index_l]
    return pre_total_mileage,pre_mileage
...
if __name__ == "__main__":
    ...
    pkl_filename = "mileage_pickle_model.pkl"
    ...
    # index_r = index_l + 100
    pre_total_mileage,pre_mileage = pre_data(df,index_l,index_r,rf_end_V,rf,index_r)
    # 预测可行驶的里程值
    plt.figure()
    ...
    for i in range(index_l+1+old_drive,index_r+1):
        pre_total_mileage, pre_mileage = pre_data(df, index_l+old_drive, i, rf_end_V, rf,index_r)
