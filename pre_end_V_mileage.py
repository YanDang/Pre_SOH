...
if __name__ == "__main__":
    ...
    rf_end_V = sk.ensemble.RandomForestRegressor()
    ...
    for file_No in range(4,10):
        ...
        if file_No != 9:
            for i in range(len(df_data)):
                x_train.append([V_y_pre[index], df_data['最低温度'][i], df_data['起始电压'][i],df_data['SOC差值'][i]])
                index += 1
            y_train += list(df_data['里程差值'])
        if file_No == 9:
            for i in range(len(df_data)):
                x_test.append([df_data['终止电压'][i],df_data['最低温度'][i],df_data['起始电压'][i],df_data['SOC差值'][i]])
            y_test += list(df_data['里程差值'])
